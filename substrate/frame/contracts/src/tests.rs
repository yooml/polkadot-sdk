...
use pallet_contracts_uapi::ReturnErrorCode as RuntimeReturnCode;
...
mod run_tests {
	...
	fn locking_delegate_dependency_works() {
		...
		ExtBuilder::default().existential_deposit(ED).build().execute_with(|| {
			...
			let CodeUploadReturnValue { deposit, .. } = Contracts::bare_upload_code(
				RuntimeOrigin::signed(ALICE),
				wasm_callee.clone(),
				deposit_limit::<Test>(),
			)
	...
	fn read_only_subsequent_call_cannot_store() {
		...
		ExtBuilder::default().existential_deposit(200).build().execute_with(|| {
			...
			let input =
				(&addr_callee, pallet_contracts_uapi::CallFlags::empty().bits(), 0u64, 100u32);
