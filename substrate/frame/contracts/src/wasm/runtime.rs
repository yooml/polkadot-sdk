...
use crate::{
	...
	weights::WeightInfo,
	BalanceOf, CodeHash, Config, DebugBufferVec, Error, LOG_TARGET, SENTINEL,
};
...
use pallet_contracts_uapi::{CallFlags, ReturnErrorCode, ReturnFlags, StorageFlags};
...
impl<'a, E: Ext, M: ?Sized + Memory<E::T>> Runtime<'a, E, M> {
	...
	fn decode_key(&self, memory: &M, key_ptr: u32, key_len: u32) -> Result<Key<E::T>, TrapReason> {
...
pub mod env {
	...
	/// See [`pallet_contracts_uapi::HostFn::call`].
	#[api_version(0)]
	fn call(
		&mut self,
		memory: &mut M,
	...
	/// See [`pallet_contracts_uapi::HostFn::weight_left`].
	#[api_version(0)]
	fn weight_left(
		&mut self,
		memory: &mut M,
	...
	fn deposit_event(
		...
		if topics.len() > self.ext.schedule().limits.event_topics as usize {
	...
	/// See [`pallet_contracts_uapi::HostFn::debug_message`].
	#[api_version(0)]
	fn debug_message(
		&mut self,
		memory: &mut M,
